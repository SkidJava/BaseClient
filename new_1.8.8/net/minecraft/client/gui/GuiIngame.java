...
public class GuiIngame extends Gui {
	...
	// TODO:
	private float anim = 22;
	private int oldItem = 0;
	private float delay = 1000;
	private float speed = 150;
	private TimeHelper fpsTimeHelper = new TimeHelper(), timeHelper = new TimeHelper();
	...
	public void renderExpBar(ScaledResolution p_175176_1_, int p_175176_2_) {
		...
		if (i > 0) {
			...
			int j = (int) (p_175176_1_.getScaledHeight() - 32 + 3 + anim);
		...
		if (this.mc.thePlayer.experienceLevel > 0) {
			...
			int l = (int) (p_175176_1_.getScaledHeight() - 31 - 4 + anim);
			boolean flag = false;
	...
	private void renderPlayerStats(ScaledResolution p_180477_1_) {
		if (this.mc.getRenderViewEntity() instanceof EntityPlayer) {
			...
			int k1 = (int) (p_180477_1_.getScaledHeight() - 39 + anim);
	...
	private void renderVignette(float p_180480_1_, ScaledResolution p_180480_2_) {
		if (Config.isVignetteEnabled()) {
			...
			// TODO:Damage
			if (Client.getClient().getManagers().getManager(SettingManager.class).getSetting(ScreenEffect.class, "DamageEffect").getBooleanValue()
					&& playerHealth - lastPlayerHealth < 0)
				f = 1.0f;
